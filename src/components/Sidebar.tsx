    ...
    <>
      ...
      <aside className={`fixed left-0 top-0 h-screen w-64 border-r border-white/[0.03] bg-[#050505]/95 backdrop-blur-3xl flex flex-col pt-8 pb-6 px-5 z-50 transition-all duration-500 ease-in-out lg:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className="flex items-center justify-between mb-10 px-2">
          <div className="flex items-center gap-3.5 relative group cursor-pointer">
          ...
          <button 
            onClick={onClose}
            className="lg:hidden p-2 rounded-xl bg-white/5 hover:bg-white/10 text-white/50 hover:text-white transition-all"
          >
            <Zap size={18} className="rotate-90" />
          </button>
        </div>
              ...
              {group.items.map((item) => {
                ...
                return (
                  <Link 
                    ...
                    className="relative px-3 py-2.5 rounded-xl flex items-center gap-3.5 group transition-all duration-300 hover:bg-white/[0.04]"
                  >
                    {isActive && (
                      <motion.div
